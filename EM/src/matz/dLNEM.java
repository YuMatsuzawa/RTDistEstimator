...
/* EMアルゴリズムでモデル推定するクラス。
 * これはdelayedLognormalMM（時間遅れ付き混合対数正規分布）。clusterMaxは本来的にはパラメータなのでここを回して、
...
public class dLNEM {
	...
	private static String aiclogFileName = "aic_dln.log";
	private static String biclogFileName = "bic_dln.log";
		...
		try {
			...
			for (File dir : dataDirs) {
				...
				for (String fileName : dataFileNames) {
					int resFileNum = 1;
					String resFileName = fileName.replaceAll("([\\d]+\\.)(csv)", "$1res\\.dln\\." + resFileNum + "\\.$2");
					...
					if (clusteringMethod == KMEANS){ //最大クラスタ数を指定して、クラスタ数を増やしながら試行する（K平均法でクラスタ分け）
						for (int cluster=1; cluster <= clusterMax; cluster++){
							while (resFile.exists()){
								...
								resFileNum++;
								resFileName = fileName.replaceAll("([\\d]+\\.)(csv)", "$1res\\.dln\\." + resFileNum + "\\.$2");
								resFile = new File(resDir, resFileName);
							}
							if (!resFile.exists()) { 
								File dataFile = new File(dir, fileName); //入力元
								new dLNEM(cluster, dataFile, resFile);
							}
						}
						...
						if (!resFile.exists()){
							File dataFile = new File(dir, fileName); //入力元
							new dLNEM(clusteringMethod, dataFile, resFile);
						}
					}
	...
	public static void icLogRename(String iclogFileName, File resFile) {
		...
		File iclogFileRenameTo = new File(resFile.getParentFile(), resFile.getName().replaceAll("([\\d]+\\.)(res\\.dln\\.[\\d]+\\.)(csv)", "$1" + icString +"\\." + iclogFileNum + "\\.$3"));
		while (iclogFileRenameTo.exists()){
			//出力ファイルが存在する(既に1回以上処理済み)なら番号をインクリメントする。
			iclogFileNum++;
			iclogFileRenameTo = new File(resFile.getParentFile(), resFile.getName().replaceAll("([\\d]+\\.)(res\\.dln\\.[\\d]+\\.)(csv)", "$1" + icString +"\\." + iclogFileNum + "\\.$3"));
		}
	...
	public dLNEM(String[] args) { //初期化コンストラクタ
		try {
			/* 引数の順序は固定とする．
	...
	public dLNEM(int cluster, File dataFile, File resFile) throws FileNotFoundException, IOException { //計算コンストラクタ。見ての通り入力データファイルと出力先ファイルを指定する。
		...
		//プロファイル尤度を用いて時間遅れ最尤値を推定
		int tauResol = 1000;//tau候補列の解像度
		double delta = ((double)dataMax - (double)dataMin) / (double)tauResol;//連続するtau候補の差（幅）
		double[] maxPL = new double[cluster];
		double[] bestTau = new double[cluster];
		int[] memberNum = new int[cluster];
		double[] mu = new double[cluster];
		double[] sigma = new double[cluster];
		for(int k=0; k<cluster; k++){ //init
			maxPL[k] = Double.NEGATIVE_INFINITY;
			//bestTau[k] = (double)dataMin - pseudoZero;
			bestTau[k] = 0.0;
		}
		for(int r=0; r < tauResol; r++){ //itrator
			...
			//double tmpTau = (double)dataMin - pseudoZero + r*delta; //tau候補
			double tmpTau = r*delta;
			
			double[] delay = new double[size];
			for(int i=0; i<size; i++){
				int index = initCluster[i];
				delay[i] = data[i] - tmpTau;
				memberNum[index]++;
				mu[index] += Math.log(delay[i]);
			}
			for(int k=0; k<cluster; k++){
				if(memberNum[k]==0) continue;
				mu[k] /= memberNum[k];
			}
			for(int i=0; i<size; i++){
				int index = initCluster[i];
				sigma[index] += Math.log(delay[i]) - mu[index];
			}
			for(int k=0; k<cluster; k++){
				if(memberNum[k]<2) continue;
				sigma[k] /= memberNum[k];
			}
			for(int k=0; k<cluster; k++){
				double tmpPL = - memberNum[k] * (mu[k] + Math.log(sigma[k]));
				if(!Double.isNaN(tmpPL) && tmpPL > maxPL[k]) {
					maxPL[k] = tmpPL;
					bestTau[k] = tmpTau;
				}
			}
		...
		double[] trans = new double[cluster];
		for (int k=0; k<cluster; k++) {
			weight[k] = 0.0;
			mean[k] = 0.0;
			var[k] = 0.0;
		...
		for(int i=0; i<size; i++){
			...
			dataNum[index]++;
			weight[index]++;
			double delay = data[i] - trans[index];
			/*if(delay >= 1) mean[index] += Math.log(delay);   //!!!
			else mean[index] += Math.log(altDelay);*/
			mean[index] += Math.log(delay);
		}
		for(int k=0; k<cluster; k++) {
		...
		for(int i=0; i<size; i++) {
			int index = initCluster[i];
			double delay = data[i] - trans[index];
			double gap = Math.log(delay) - mean[index];
			/*if(delay >= 1) gap = Math.log(delay) - mean[index];
		...
		for(int itr=0; itr<maxIter; itr++){
				...
				for(int k=0; k<cluster; k++){
					try {
						if(var[k]==0) throw new Exception();
							...
							eAff[i][k] = weight[k] * Math.exp(-0.5*gap*gap/var[k])/(cnst*Math.sqrt(var[k]))/delay;
						}
			...
			for(int k=0; k<cluster; k++){
				...
				for(int i=0; i<size; i++){
					if(data[i]<=trans[k]){
						...
						double delay = data[i] - trans[k];
						//if (delay < 1) delay = altDelay;
						mean[k] += Math.log(delay)*eAff[i][k];
					}
				}
			...
			for(int i=0; i<size; i++){
				...
				for (int k=0; k<cluster; k++){
					double delay = data[i] - trans[k];
					if (delay > 0) { //時間遅れ付き分布では、バースト時点より前のデータは分布に属さないので尤度もゼロと見る
						//if (delay < 1) delay = altDelay;
						double gap = Math.log(delay) - mean[k];
		...
		for (int r=0; r<resol; r++) {
			...
			for (int k=0; k<cluster; k++) {
				subPlot = weight[k] * delayedLognormal(time, trans[k], mean[k], var[k]);
		...
		for(int k=0; k<cluster; k++) {
			...
			bw.newLine();
			bw.write(String.format("delay: %f", trans[k]));
			bw.newLine();
			bw.newLine();
		}
		...
		//criterion出力
		if (criterion == AIC) AIC(cluster, size, data, trans, mean, var, weight, resFile);
		else if (criterion == BIC) BIC(cluster, size, data, trans, mean, var, weight, resFile);
	}
	
	private void BIC(int cluster, int size, double[] data, double[] trans,
			double[] mean, double[] var, double[] weight, File resFile) {//BIC出力
		try {
			...
			int numOfParam = 4; //クラスタあたりのパラメータ数
	...
	private void AIC(int cluster, int size, double[] data, double[] trans,
			double[] mean, double[] var, double[] weight, File resFile) { //AIC出力
		try {
